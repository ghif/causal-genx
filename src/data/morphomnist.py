...
# New provider boundary. The older `morphomnist(args)` API remains compatible.
MORPHOMNIST_SCHEMA = CausalGraphSpec(
    dataset_id="morphomnist",
    version="1",
    variables=(
        VariableSpec("thickness", VariableKind.CONTINUOUS, normalization="[-1,1]"),
        VariableSpec("intensity", VariableKind.CONTINUOUS, normalization="[-1,1]"),
        VariableSpec("digit", VariableKind.CATEGORICAL, encoded_dim=10),
    ),
    edges=(("digit", "thickness"), ("thickness", "intensity")),
)


class MorphoMNISTProvider:
    schema = MORPHOMNIST_SCHEMA

    def __init__(self, root: str, input_res: int = 32, pad: int = 4, context_norm: str = "[-1,1]"):
        self.root, self.input_res, self.pad, self.context_norm = root, input_res, pad, context_norm

    @property
    def spec(self) -> DatasetSpec:
        return DatasetSpec("morphomnist", self.root, ImageSpec(1, self.input_res, self.input_res), {"train": "train", "valid": "t10k", "test": "t10k"}, "{split}-morpho.csv", {"train_pad": self.pad, "eval_pad": 2})

    def load_split(self, split: str) -> MorphoMNIST:
        if split not in {"train", "valid", "test"}:
            raise ValueError(f"Unknown split {split!r}")
        return MorphoMNIST(self.root, train=split == "train", columns=list(self.schema.variable_names), norm=self.context_norm, concat_pa=False, pad=self.pad, input_res=self.input_res)
    ...
    def make_batch(self, split: str, indices: Sequence[int], *, rng=None, training: bool = False) -> Batch:
        ...
        return Batch(np.asarray(raw["x"], dtype=np.float32), {name: np.asarray(raw[name], dtype=np.float32) for name in self.schema.variable_names})

    def fingerprint(self) -> str:
        value = f"{self.root}|{self.input_res}|{self.pad}|{self.context_norm}|{self.schema.version}"
        return hashlib.sha256(value.encode()).hexdigest()[:16]


_DATASET_FACTORIES: dict[str, Callable[..., MorphoMNISTProvider]] = {"morphomnist": MorphoMNISTProvider}


def create_dataset(name: str, **kwargs) -> MorphoMNISTProvider:
    try:
        return _DATASET_FACTORIES[name](**kwargs)
    except KeyError as exc:
        raise KeyError(f"Unknown dataset {name!r}; available={sorted(_DATASET_FACTORIES)}") from exc
