...
def _variable_value(variable):
    return variable.get_value() if hasattr(variable, "get_value") else variable.value


def _set_variable_value(variable, value) -> None:
...
class Encoder(nnx.Module):
    def __init__(
        ...
        for block in blocks:
            ...
            _set_variable_value(last.kernel, _variable_value(last.kernel) * scale)
...
class Decoder(nnx.Module):
    ...
    def _scale_weights(self):
        ...
        for block in self.blocks:
            _set_variable_value(
                block.z_proj.kernel,
                _variable_value(block.z_proj.kernel) * scale,
            )
            conv_kernel = _last_conv(block.conv).kernel
            _set_variable_value(conv_kernel, _variable_value(conv_kernel) * scale)
            prior_kernel = _last_conv(block.prior).kernel
            _set_variable_value(prior_kernel, _variable_value(prior_kernel) * 0.0)
...
# Stable names let scripts/configs select image mechanisms without importing a
# dataset-specific training entrypoint.
MODEL_REGISTRY: dict[str, Callable] = {
    "hierarchical_vae": HVAE,
    "simple_vae": SimpleVAE,
}
