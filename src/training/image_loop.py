...
def make_train_step(graphdef, tx, ema_decay: float, grad_skip: float):
    def _step(params, opt_state, ema_params, batch, beta, rng):
        ...
        valid = jnp.logical_and(valid, grad_norm < grad_skip)
        ...
        out = {
            **out,
            "grad_norm": grad_norm,
            "update_skipped": jnp.logical_not(valid).astype(jnp.float32),
        }
...
def make_pmap_train_step(graphdef, tx, ema_decay: float, grad_skip: float, devices):
    def _step(params, opt_state, ema_params, batch, beta, rng):
        ...
        local_valid = jnp.logical_and(local_valid, grad_norm < grad_skip)
        valid = jax.lax.pmin(local_valid.astype(jnp.int32), axis_name="devices").astype(jnp.bool_)
        ...
        for epoch in range(state.epoch, args.epochs):
            ...
            for step in range(steps_per_epoch):
                ...
                if run_steps == 0:
                    ...
                    logger.info(
                        "compiling_first_train_step=started; this cold XLA compile can take time and is cached for later runs"
                    )
                ...
                if use_tpu_pmap:
                    ...
                    _block_until_ready(out)
                    metric_out = out
                update_skipped = bool(float(metric_out["update_skipped"]))
                if update_skipped:
                    # Match the PyTorch trainer: the compiled function computes
                    # a candidate update, while the host keeps all prior state
                    # when the scalar loss/gradient safety check fails. Keeping
                    # this full-tree selection outside XLA avoids a huge HLO.
                    new_params = state.params
                    new_opt_state = state.opt_state
                    new_ema_params = state.ema.params
                if first_compile_started is not None:
                    logger.info(
                        "compiling_first_train_step=completed duration=%.1fs",
                        time.perf_counter() - first_compile_started,
                    )
                    first_compile_started = None
                state = TrainState(
                    params=new_params,
